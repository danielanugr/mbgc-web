...
import { generateSEOMetadata, generateEventJsonLd } from "@/lib/seo";
...
export async function generateMetadata({
  ...
  if (!event) return generateSEOMetadata();

  return generateSEOMetadata({
    title: event.title || "Event",
    description: `Ikuti ${event.title || "Event"} bersama Mataram Board Game Community.`,
    image: event.image
      ? urlForImage(event.image).width(1200).height(630).url()
      : undefined,
    url: `/events/${resolvedParams.slug}`,
    type: "article",
  });
}
...
export default async function EventDetailPage({
  ...
  const jsonLd = generateEventJsonLd({
    ...
    startDate: eventDate ? eventDate.toISOString() : new Date().toISOString(),
    image: event.image ? urlForImage(event.image).url() : undefined,
    location: event.location || "Mataram",
  });
    ...
    <>
      ...
      <main className='flex-1 flex flex-col items-center pb-24 pt-8'>
        <div className='container-fluid w-full max-w-6xl'>
          <Link
            href='/events'
            className='inline-flex items-center text-primary font-bold hover:text-accent-orange mb-8 group transition-colors bg-white px-4 py-2 rounded-xl border-2 border-primary shadow-[2px_2px_0px_0px_#162836] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 active:shadow-none active:translate-x-1 active:translate-y-1'
          >
            ...
            <div className='lg:col-span-7 bg-white border-4 border-primary rounded-3xl p-8 md:p-12 shadow-[8px_8px_0px_0px_#162836]'>
              <div className='mb-10'>
                <div className='flex justify-between items-start gap-4 mb-8'>
                  ...
                  {eventDate && (
                    <div className='hidden sm:flex bg-primary text-white font-black font-display text-center rounded-2xl border-4 border-primary shadow-[4px_4px_0px_0px_#dba58a] transform rotate-3 flex-col w-28 shrink-0'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 bg-primary/5 p-6 rounded-2xl border-2 border-primary border-dashed'>
                  ...
                  {eventDate && (
                    <div className='flex items-center text-primary font-bold'>
                      <div className='w-12 h-12 rounded-xl bg-accent-orange/50 border-2 border-primary flex items-center justify-center mr-4 shrink-0'>
                  ...
                  {event.location && (
                    <div className='flex items-center text-primary font-bold sm:col-span-2 mt-2 pt-4 border-t-2 border-primary/10 border-dashed'>
                      <div className='w-12 h-12 rounded-xl bg-white border-2 border-primary flex items-center justify-center mr-4 shrink-0'>
              ...
              <div className='prose prose-lg prose-headings:font-display prose-headings:font-black prose-headings:text-primary prose-a:text-accent-orange prose-a:font-bold prose-strong:font-black max-w-none text-primary/80'>
                {event.description ? (
                  ...
                  <p className='italic text-primary/50'>
                    Tidak ada deskripsi untuk event ini. Langsung datang ke
                    lokasi aja dan main bareng!
                  </p>
                )}
              </div>
              ...
              <div className='mt-12 pt-8 border-t-4 border-primary border-dashed flex flex-col sm:flex-row items-center gap-4'>
                ...
                <button
                  type='button'
                  className='w-full sm:w-auto px-6 py-4 bg-white text-primary font-display font-bold text-lg text-center rounded-xl border-4 border-primary shadow-[6px_6px_0px_0px_#dba58a] hover:translate-y-1 hover:translate-x-1 hover:shadow-[2px_2px_0px_0px_#dba58a] transition-all flex items-center justify-center'
                >
