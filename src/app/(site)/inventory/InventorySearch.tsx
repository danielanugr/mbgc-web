...
export function InventorySearch({ initialQuery }: { initialQuery: string }) {
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      if (debouncedQuery) {
        router.push(`/inventory?q=${encodeURIComponent(debouncedQuery)}`, {
          scroll: false,
        });
      } else {
  ...
  return (
    <div className='relative w-full max-w-3xl mx-auto mb-16 group'>
      <div className='absolute -inset-2 bg-linear-to-r from-accent-orange to-accent-peach rounded-3xl blur-xl opacity-30 group-hover:opacity-50 transition-opacity'></div>
      <div
        className={`relative flex overflow-hidden bg-white border-4 border-primary rounded-2xl shadow-[6px_6px_0px_0px_#162836] focus-within:shadow-[6px_6px_0px_0px_#cf7650] focus-within:-translate-y-1 transition-all h-20 ${isPending || query !== debouncedQuery ? "opacity-70" : ""}`}
      >
        <div className='pl-6 text-primary flex items-center justify-center'>
          <Search
            size={28}
            className={
              isPending || query !== debouncedQuery ? "animate-pulse" : ""
            }
          />
        </div>
        <input
