...
import { generateSEOMetadata, generateBoardGameJsonLd } from "@/lib/seo";

export const revalidate = 60;

export const metadata = generateSEOMetadata({
  title: "Koleksi Board Game",
  description:
    "Daftar koleksi board game yang dapat dimainkan di Mataram Board Game Community.",
  url: "/inventory",
});
...
export default async function InventoryPage({
  ...
  const jsonLdData = initialGames.map(
    (game: any /* eslint-disable-line @typescript-eslint/no-explicit-any */) =>
      generateBoardGameJsonLd({
        name: game.title,
        description: game.shortDescription || game.title,
        image: game.image ? urlForImage(game.image).url() : undefined,
          ...
          <section className='bg-primary border-4 border-primary rounded-3xl p-8 md:p-16 shadow-[8px_8px_0px_0px_#162836] relative overflow-hidden mb-16 text-white'>
            ...
            <div className='relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left flex-1'>
              <div className='flex-1'>
                <div className='inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-accent-peach border-2 border-primary text-primary font-bold font-display mb-6 shadow-[4px_4px_0px_0px_#162836] -rotate-2'>
                ...
                <p className='text-xl md:text-2xl font-bold text-white/90 max-w-xl mx-auto md:mx-0 leading-relaxed mt-8'>
                  Eksplorasi koleksi board game yang tersedia di lemari MBGC.
                  Dari game party buat ketawa sampai euro game buat mikir keras!
                </p>
              </div>
